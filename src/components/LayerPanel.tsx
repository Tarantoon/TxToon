...
function SortableLayerCard({ layer, index }: SortableLayerCardProps) {
  ...
  return (
    <div
      ref={ref}
      className={`border-2 border-black bg-white transition-[opacity,box-shadow,transform] duration-150 ${activeLayerId === layer.id ? 'shadow-[4px_4px_0_#000]' : ''} ${isDropTarget && !isDragging ? 'outline-4 outline-offset-2 outline-black' : ''} ${isDragging ? 'opacity-30' : 'opacity-100'}`}
    >
      <div
        className={`flex border-b-2 border-black font-mono ${activeLayerId === layer.id ? 'bg-black text-white' : 'bg-white'}`}
      >
        <button
          ref={handleRef}
          type="button"
          className="flex w-8 cursor-grab touch-none items-center justify-center border-r-2 border-black text-xs active:cursor-grabbing"
          aria-label={`Drag ${layer.name} to reorder`}
          title="Drag or use keyboard to reorder"
        >
          ≡
        </button>
        <button
...
export function LayerPanel() {
  ...
  const draggedLayer = layers.find((layer) => layer.id === draggedLayerId)
  ...
  const handleDragStart = (event: DragStartEvent) => {
    setDraggedLayerId(String(event.operation.source?.id ?? ''))
  }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!event.canceled && sourceId != null && targetId != null) {
      reorderLayer(String(sourceId), String(targetId))
    }
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-neutral-200 p-2">
        ...
        >
          <div className="flex flex-col gap-2">
            {layersTopToBottom.map((layer, index) => (
              <SortableLayerCard key={layer.id} layer={layer} index={index} />
            ))}
          </div>
          <DragOverlay>
            {draggedLayer ? (
              <div className="border-2 border-black bg-black px-3 py-2 font-mono text-[10px] font-black tracking-wider text-white shadow-[5px_5px_0_rgba(0,0,0,0.35)]">
                {draggedLayer.type === 'ascii' ? 'TXT' : 'IMG'} / {draggedLayer.name}
              </div>
            ) : null}
          </DragOverlay>
        </DragDropProvider>
